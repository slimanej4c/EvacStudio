...
import { FilePlus2, Files, Gauge, LogOut, ShieldCheck, Users, X } from "lucide-react";
...
const planItems: NavItem[] = [
  ...
  { label: "Partage", href: "/partage", icon: Users, activePattern: /^\/partage$/ },
];
    ...
    <Link
      ...
      className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
        active
          ...
          : "text-stone-600 hover:bg-brand-cream hover:text-brand-red"
      }`}
    >
...
export function AppSidebar() {
  ...
  if (sidebarHidden) {
    return (
      <button
        ...
        className="fixed left-4 top-4 z-50 inline-flex h-11 w-11 items-center justify-center rounded-xl border border-brand-orange/25 bg-white shadow-lg transition-transform hover:scale-105"
  ...
  return (
    <aside className="sticky left-0 top-0 flex h-screen w-72 shrink-0 flex-col border-r border-brand-orange/15 bg-[#fffdf9]">
      <div className="flex h-20 items-center justify-between gap-3 border-b border-brand-orange/15 px-5">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-14 w-40 shrink-0 items-center overflow-hidden rounded-lg bg-white">
        ...
        <button
          ...
          className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-stone-500 transition-colors hover:bg-brand-cream hover:text-brand-red"
      ...
      <nav ref={navRef} className="flex-1 space-y-2 overflow-y-auto px-4 py-5">
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-brand-orange/20 bg-brand-cream px-3 py-2 text-[11px] font-extrabold uppercase tracking-normal text-brand-red">
      ...
      <div className="border-t border-brand-orange/15 p-4">
        <div className="mb-3 rounded-lg bg-brand-cream px-3 py-2">
          <p className="truncate text-sm font-semibold text-brand-ink">{displayName}</p>
          <p className="truncate text-xs text-stone-500">{user?.email || "Utilisateur connecté"}</p>
        </div>
        <button
          onClick={logout}
          className="flex w-full items-center justify-center gap-2 rounded-lg border border-brand-red/20 bg-red-50 px-3 py-2 text-sm font-medium text-brand-red transition-colors hover:bg-red-100"
        >
