        ...
        {loading ? (
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {plans.map((plan) => {
              ...
              return (
              <article key={plan.id} className="group overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md">
                <Link href={`/evacuation-plans/${plan.id}/editor`} className="block">
                  <div className="relative flex h-44 items-center justify-center overflow-hidden border-b border-slate-100 bg-slate-50">
                    ...
                    <span className="absolute right-2 top-2 rounded-full bg-safety-green/95 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white shadow-sm">
                      {plan.background_type.toUpperCase()}
                    </span>
                  </div>
                  <div className="p-4">
                    <h2 className="truncate text-base font-semibold text-slate-950">{plan.title}</h2>
                ...
                <div className="flex items-center justify-between border-t border-slate-100 px-4 py-2.5">
                  ...
                  <div className="flex items-center gap-3">
                    <a href={plan.background_file} target="_blank" rel="noreferrer" title="Voir le fond de plan" className="text-slate-400 hover:text-safety-green">
